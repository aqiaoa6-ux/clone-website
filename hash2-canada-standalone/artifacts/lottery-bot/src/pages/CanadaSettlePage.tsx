...
import { api, type CanadaConfig, type CanadaPlan } from "../lib/api";
...
function makeFallbackPlan(index: number): CanadaPlan {
  return {
    id: `plan-${index + 1}`,
    name: `方案${index + 1}`,
    enabled: false,
...
function makeFallbackConfig(): CanadaConfig {
  return {
...
function getBetOdds(plan: CanadaPlan, key: string): number {
...
export default function CanadaSettlePage() {
  const { user, logout } = useAuth();
  const [, setLocation] = useLocation();
  const [config, setConfig] = useState<CanadaConfig>(makeFallbackConfig());
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const cfg = await api.canada.config();
        if (!mounted) return;
        setConfig(cfg.plans?.length ? cfg : makeFallbackConfig());
      } catch (e) {
        if (!mounted) return;
        setConfig(makeFallbackConfig());
        setErrorMessage(e instanceof Error ? e.message : "读取加拿大配置失败");
      } finally {
      ...
      <div className="sticky top-0 z-40 bg-[#0b0e1a]/95 border-b border-[#1e2235] backdrop-blur">
        <div className="max-w-lg mx-auto px-4 py-3 flex justify-between items-center">
          <div className="flex items-center gap-2">
            ...
            <div>
              <div className="font-bold text-white">加拿大结算</div>
