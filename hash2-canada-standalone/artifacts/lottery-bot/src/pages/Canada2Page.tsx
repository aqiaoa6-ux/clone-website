...
function makeDefaultConfig(): CanadaConfig {
  return {
    plans: Array.from({ length: 8 }, (_, i) => makeDefaultPlan(i)),
    updatedAt: Date.now(),
  };
}

export default function Canada2Page() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const [cfg, tg] = await Promise.all([api.canada2.config(), api.tg.status()]);
        if (!mounted) return;
            ...
            >
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    <CanadaPlanStatField activePlanId={currentPlan.id} label="最高未中金额" valueKey="maxMissAmount" />
                  </div>
                  <div>
                    <CanadaPlanStatField activePlanId={currentPlan.id} label="最高中奖金额" valueKey="maxWinAmount" />
                  </div>
                  <div>
...
function CanadaLiveOverview({
  ...
  const fetchRuntime = useCallback(async () => {
    try {
      const rt = await api.canada2.runtime();
      setRuntime(prev => {
...
function CanadaPlanRuntimeSummary({
  ...
  const resetRuntime = async () => {
    setResetting(true);
    try {
      const res = await api.canada2.resetRuntime(activePlanId);
